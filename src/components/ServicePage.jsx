    ...
    <div className="min-h-screen bg-white text-gray-900 selection:bg-blue-500/30 overflow-x-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 py-10 relative z-10">
        ...
        <Motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
          <Link to="/services" className="group inline-flex items-center gap-3 text-gray-600 hover:text-gray-900 transition-all">
            <div className={`p-2 rounded-full border border-gray-200 ${p.border}`}>
              <ArrowLeft size={18} />
            </div>
        ...
        <section className="mb-12">
          <div className="grid lg:grid-cols-2 gap-10 items-start mb-10">
            <Motion.div {...fadeInUp} className="space-y-6">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <span className="px-4 py-2 rounded-full border border-gray-200 text-gray-600 text-[10px] font-black uppercase tracking-widest">
                  Recurring Revenue
                </span>
                <span className="px-4 py-2 rounded-full border border-gray-200 text-gray-600 text-[10px] font-black uppercase tracking-widest">
                  Caribbean Ready
            ...
            <Motion.div {...fadeInUp} className="grid grid-cols-2 gap-4">
              {[
                ...
                <div key={i} className="bg-gray-50 border border-gray-200 rounded-3xl p-6">
                  ...
                  <h4 className="text-gray-900 font-bold">{item.val}</h4>
                  <p className="text-gray-500 text-sm">{item.lbl}</p>
                </div>
              ))}
            </Motion.div>
          </div>
          ...
          <div className="mb-10">
            <div className="flex items-end justify-between mb-10 gap-4">
              ...
              <div className={`h-[1px] flex-grow bg-gray-100 hidden md:block max-w-xs`} />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {service.features.map((f, i) => (
                <Motion.div
                  ...
                  whileHover={{ y: -8 }}
                  className="group p-7 bg-gray-50 border border-gray-100 rounded-3xl transition-all duration-500"
                >
                  <span className={`block w-8 h-0.5 ${p.bar} mb-6 rounded-full`} />
                  <h4 className="text-lg font-bold text-gray-800 mb-2 group-hover:text-gray-900">{f.title}</h4>
        ...
        {/* Deliverables */}
        <section className="mb-12">
          <div className="relative rounded-[3rem] border border-gray-200 bg-gray-50 p-10 md:p-16 overflow-hidden">
            ...
            <div className="grid lg:grid-cols-2 gap-12 relative z-10">
              <div>
                ...
                <Link
                  to="/contact"
                  className="group relative inline-flex items-center gap-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-8 py-4 rounded-full font-bold text-sm uppercase tracking-widest shadow-xl shadow-blue-600/25 transition-all hover:pr-10 hover:shadow-blue-600/40"
                >
                  Start Your Project
        ...
        <section className="mb-12">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-500 mt-4 max-w-xl mx-auto">Every package is tailored to your goals. Talk to us and we'll scope the perfect fit.</p>
          </div>
            ...
            {service.packages.map((pkg, i) => (
              <Motion.div
                ...
                className={`relative rounded-[2.5rem] p-8 border transition-all duration-500 flex flex-col ${
                  i === 1
                    ? 'bg-gradient-to-br from-blue-50 to-violet-50 border-blue-200 shadow-2xl shadow-blue-600/10'
              ...
              {related.map((s) => {
                ...
                return (
                  <Link
                    key={s.slug}
                    to={`/services/${s.slug}`}
                    className="group p-7 bg-white border border-gray-200 rounded-3xl hover:border-blue-400/50 hover:shadow-xl hover:shadow-blue-600/10 transition-all duration-500"
                  >
                    <div className={`w-12 h-12 rounded-xl ${rp.soft} flex items-center justify-center mb-5 ${rp.text}`}>
                      <RelatedIcon size={22} />
                    </div>
                    <h4 className="text-lg font-bold text-gray-900 group-hover:text-gray-900 mb-2">{s.name}</h4>
        ...
        <section className="mb-8 text-center relative">
          ...
          <Motion.div {...fadeInUp} className="space-y-8">
            ...
            <p className="text-gray-600 max-w-xl mx-auto text-lg">Let's talk about your project and build something your customers will love.</p>
            <Link
              to="/contact"
              className={`group relative inline-flex items-center gap-4 ${p.solid} ${p.solidHover} text-white px-12 py-6 rounded-full font-bold transition-all hover:pr-14`}
            >
